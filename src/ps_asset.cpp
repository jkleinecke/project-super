...
internal void
AddPipelineToRenderManifest(render_manifest* manifest, pipeline_asset* pipeline)
{
    u32 index = manifest->pipelineCount++;
    manifest->pipelines[index] = pipeline->desc;
    manifest->pipelines[index].id = pipeline->id;
}
